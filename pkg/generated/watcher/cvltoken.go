...
// File was generated at 2019-12-05 18:54:54.322831 +0000 UTC
package watcher
...
func NewCVLTokenContractWatchersAllEvents(contractAddress common.Address) *CVLTokenContractWatchers {
	return &CVLTokenContractWatchers{
		contractAddress: contractAddress,
		enableAll:       true,
	}
...
func (w *CVLTokenContractWatchers) StartCVLTokenContractWatchers(client bind.ContractBackend,
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("CVLTokenContract", "Approval") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("CVLTokenContract", "OwnershipRenounced") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("CVLTokenContract", "OwnershipTransferred") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("CVLTokenContract", "Transfer") {
	...
	return utils.NewWatcherSubscription("WatchApproval", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting Approval: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old Approval")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart Approval: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchOwnershipRenounced", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting OwnershipRenounced: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old OwnershipRenounced")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart OwnershipRenounced: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchOwnershipTransferred", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting OwnershipTransferred: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old OwnershipTransferred")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart OwnershipTransferred: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchTransfer", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting Transfer: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old Transfer")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart Transfer: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
