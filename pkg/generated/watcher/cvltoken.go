...
// File was generated at 2019-03-05 02:17:47.608581 +0000 UTC
package watcher
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CVLTokenContractApproval, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchApproval")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchApproval")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchApproval: %v", err)
					continue
				}
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CVLTokenContractTransfer, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchTransfer")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchTransfer")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchTransfer: %v", err)
					continue
				}
				return sub, recvChan, nil
			}
