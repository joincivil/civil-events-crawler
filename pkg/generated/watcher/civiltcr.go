...
// File was generated at 2018-07-20 19:06:01.569924645 +0000 UTC
package watcher
...
func (w *CivilTCRContractWatchers) startWatchAppealGranted(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_AppealGranted", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
...
func (w *CivilTCRContractWatchers) startWatchAppealRequested(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_AppealRequested", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplication(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_Application", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplicationRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_ApplicationRemoved", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplicationWhitelisted(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_ApplicationWhitelisted", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallenge(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_Challenge", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallengeFailed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_ChallengeFailed", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallengeSucceeded(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_ChallengeSucceeded", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchDeposit(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_Deposit", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchFailedChallengeOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_FailedChallengeOverturned", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGovernmentTransfered(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_GovernmentTransfered", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealChallenged(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_GrantedAppealChallenged", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealConfirmed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_GrantedAppealConfirmed", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_GrantedAppealOverturned", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchListingRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_ListingRemoved", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchListingWithdrawn(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_ListingWithdrawn", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchRewardClaimed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_RewardClaimed", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchSuccessfulChallengeOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_SuccessfulChallengeOverturned", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchTouchAndRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_TouchAndRemoved", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchWithdrawal(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("_Withdrawal", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
