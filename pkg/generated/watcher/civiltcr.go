...
// File was generated at 2018-10-11 16:19:23.566330135 +0000 UTC
package watcher
...
func (w *CivilTCRContractWatchers) startWatchAppealGranted(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("AppealGranted", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
...
func (w *CivilTCRContractWatchers) startWatchAppealRequested(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("AppealRequested", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplication(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("Application", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplicationRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("ApplicationRemoved", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplicationWhitelisted(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("ApplicationWhitelisted", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallenge(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("Challenge", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallengeFailed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("ChallengeFailed", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallengeSucceeded(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("ChallengeSucceeded", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchDeposit(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("Deposit", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchFailedChallengeOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("FailedChallengeOverturned", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGovernmentTransfered(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("GovernmentTransfered", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealChallenged(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("GrantedAppealChallenged", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealConfirmed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("GrantedAppealConfirmed", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("GrantedAppealOverturned", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchListingRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("ListingRemoved", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchListingWithdrawn(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("ListingWithdrawn", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchRewardClaimed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("RewardClaimed", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchSuccessfulChallengeOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("SuccessfulChallengeOverturned", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchTouchAndRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("TouchAndRemoved", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchWithdrawal(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				modelEvent, err := model.NewEventFromContractEvent("Withdrawal", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt64(), model.Watcher)
				if err != nil {
					log.Errorf("Error creating new event: event: %v, err: %v", event, err)
					continue
				}
