...
// File was generated at 2019-03-03 23:56:07.367112 +0000 UTC
package watcher
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of AppealGranted")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting AppealGranted: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart AppealGranted: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchAppealGranted: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchAppealRequested(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of AppealRequested")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting AppealRequested: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart AppealRequested: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchAppealRequested: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchApplication(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of Application")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting Application: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart Application: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchApplication: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchApplicationRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of ApplicationRemoved")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting ApplicationRemoved: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart ApplicationRemoved: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchApplicationRemoved: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchApplicationWhitelisted(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of ApplicationWhitelisted")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting ApplicationWhitelisted: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart ApplicationWhitelisted: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchApplicationWhitelisted: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchChallenge(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of Challenge")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting Challenge: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart Challenge: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchChallenge: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchChallengeFailed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of ChallengeFailed")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting ChallengeFailed: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart ChallengeFailed: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchChallengeFailed: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchChallengeSucceeded(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of ChallengeSucceeded")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting ChallengeSucceeded: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart ChallengeSucceeded: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchChallengeSucceeded: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchDeposit(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of Deposit")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting Deposit: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart Deposit: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchDeposit: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchFailedChallengeOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of FailedChallengeOverturned")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting FailedChallengeOverturned: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart FailedChallengeOverturned: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchFailedChallengeOverturned: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchGovernmentTransfered(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of GovernmentTransfered")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting GovernmentTransfered: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart GovernmentTransfered: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchGovernmentTransfered: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealChallenged(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of GrantedAppealChallenged")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting GrantedAppealChallenged: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart GrantedAppealChallenged: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchGrantedAppealChallenged: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealConfirmed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of GrantedAppealConfirmed")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting GrantedAppealConfirmed: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart GrantedAppealConfirmed: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchGrantedAppealConfirmed: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of GrantedAppealOverturned")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting GrantedAppealOverturned: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart GrantedAppealOverturned: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchGrantedAppealOverturned: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchListingRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of ListingRemoved")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting ListingRemoved: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart ListingRemoved: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchListingRemoved: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchListingWithdrawn(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of ListingWithdrawn")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting ListingWithdrawn: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart ListingWithdrawn: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchListingWithdrawn: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchRewardClaimed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of RewardClaimed")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting RewardClaimed: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart RewardClaimed: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchRewardClaimed: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchSuccessfulChallengeOverturned(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of SuccessfulChallengeOverturned")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting SuccessfulChallengeOverturned: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart SuccessfulChallengeOverturned: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchSuccessfulChallengeOverturned: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchTouchAndRemoved(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of TouchAndRemoved")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting TouchAndRemoved: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart TouchAndRemoved: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchTouchAndRemoved: %v", event)
...
func (w *CivilTCRContractWatchers) startWatchWithdrawal(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of Withdrawal")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting Withdrawal: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart Withdrawal: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
				log.Errorf("Received event on WatchWithdrawal: %v", event)
