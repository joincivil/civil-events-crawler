...
// File was generated at 2018-06-20 18:19:19.526571072 +0000 UTC
package watcher
...
func (w *CivilTCRContractWatchers) startWatchAppealGranted(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_AppealGranted", w.contractAddress, event)
				if err != nil {
...
func (w *CivilTCRContractWatchers) startWatchAppealRequested(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_AppealRequested", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplication(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_Application", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplicationRemoved(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_ApplicationRemoved", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchApplicationWhitelisted(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_ApplicationWhitelisted", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallenge(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_Challenge", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallengeFailed(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_ChallengeFailed", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchChallengeSucceeded(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_ChallengeSucceeded", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchDeposit(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_Deposit", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchFailedChallengeOverturned(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_FailedChallengeOverturned", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGovernmentTransfered(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_GovernmentTransfered", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealChallenged(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_GrantedAppealChallenged", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealConfirmed(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_GrantedAppealConfirmed", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchGrantedAppealOverturned(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_GrantedAppealOverturned", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchListingRemoved(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_ListingRemoved", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchListingWithdrawn(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_ListingWithdrawn", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchRewardClaimed(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_RewardClaimed", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchSuccessfulChallengeOverturned(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_SuccessfulChallengeOverturned", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchTouchAndRemoved(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_TouchAndRemoved", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *CivilTCRContractWatchers) startWatchWithdrawal(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("_Withdrawal", w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
