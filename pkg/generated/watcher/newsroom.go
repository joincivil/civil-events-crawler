...
// File was generated at 2019-12-05 18:54:49.654887 +0000 UTC
package watcher
...
func NewNewsroomContractWatchersAllEvents(contractAddress common.Address) *NewsroomContractWatchers {
	return &NewsroomContractWatchers{
		contractAddress: contractAddress,
		enableAll:       true,
	}
...
func (w *NewsroomContractWatchers) StartNewsroomContractWatchers(client bind.ContractBackend,
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "ContentPublished") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "NameChanged") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "OwnershipRenounced") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "OwnershipTransferred") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "RevisionSigned") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "RevisionUpdated") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "RoleAdded") {
	...
	if w.enableAll || specs.IsListenerEnabledForEvent("NewsroomContract", "RoleRemoved") {
	...
	return utils.NewWatcherSubscription("WatchContentPublished", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting ContentPublished: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old ContentPublished")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart ContentPublished: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchNameChanged", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting NameChanged: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old NameChanged")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart NameChanged: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchRevisionSigned", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting RevisionSigned: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old RevisionSigned")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart RevisionSigned: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchRevisionUpdated", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting RevisionUpdated: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old RevisionUpdated")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart RevisionUpdated: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchRoleAdded", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting RoleAdded: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old RoleAdded")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart RoleAdded: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
	...
	return utils.NewWatcherSubscription("WatchRoleRemoved", func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				...
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting RoleRemoved: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old RoleRemoved")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart RoleRemoved: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
