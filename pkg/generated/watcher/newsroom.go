...
// File was generated at 2018-07-01 14:53:04.343662874 +0000 UTC
package watcher
...
func (w *NewsroomContractWatchers) startWatchContentPublished(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEventFromContractEvent("ContentPublished", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
...
func (w *NewsroomContractWatchers) startWatchNameChanged(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEventFromContractEvent("NameChanged", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchOwnershipTransferred(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEventFromContractEvent("OwnershipTransferred", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRevisionSigned(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEventFromContractEvent("RevisionSigned", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRevisionUpdated(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEventFromContractEvent("RevisionUpdated", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRoleAdded(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEventFromContractEvent("RoleAdded", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRoleRemoved(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEventFromContractEvent("RoleRemoved", w.ContractName(), w.contractAddress, event, utils.CurrentEpochSecsInInt())
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
