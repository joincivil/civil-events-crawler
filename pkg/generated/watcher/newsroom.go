...
// File was generated at 2018-06-25 17:10:59.568940286 +0000 UTC
package watcher
...
func (w *NewsroomContractWatchers) startWatchContentPublished(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("ContentPublished", w.ContractName(), w.contractAddress, event)
				if err != nil {
...
func (w *NewsroomContractWatchers) startWatchNameChanged(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("NameChanged", w.ContractName(), w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchOwnershipTransferred(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("OwnershipTransferred", w.ContractName(), w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRevisionSigned(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("RevisionSigned", w.ContractName(), w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRevisionUpdated(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("RevisionUpdated", w.ContractName(), w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRoleAdded(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("RoleAdded", w.ContractName(), w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
...
func (w *NewsroomContractWatchers) startWatchRoleRemoved(eventRecvChan chan model.CivilEvent) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			select {
			case event := <-recvChan:
				civilEvent, err := model.NewCivilEvent("RoleRemoved", w.ContractName(), w.contractAddress, event)
				if err != nil {
					log.Errorf("Error creating new civil event: event: %v, err: %v", event, err)
					continue
				}
