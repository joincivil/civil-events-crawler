...
// File was generated at 2019-10-31 19:48:04.752004 +0000 UTC
package watcher
...
func (w *MultiSigWalletContractWatchers) cancelFunc(cancelFn context.CancelFunc, killCancel <-chan bool) {
}
...
func (w *MultiSigWalletContractWatchers) startWatchConfirmation(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchConfirmation", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractConfirmation, error) {
			...
			log.Infof("startupFn: Starting WatchConfirmation: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchConfirmation started: addr: %v", w.contractAddress.Hex())
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of Confirmation")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting Confirmation: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old Confirmation")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart Confirmation: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
...
func (w *MultiSigWalletContractWatchers) startWatchExecution(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchExecution", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractExecution, error) {
			...
			log.Infof("startupFn: Starting WatchExecution: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchExecution started: addr: %v", w.contractAddress.Hex())
			return sub, recvChan, nil
		}
		sub, recvChan, err := startupFn()
		if err != nil {
		...
		for {
			...
			// 30 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of Execution")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting Execution: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old Execution")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart Execution: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
...
func (w *MultiSigWalletContractWatchers) startWatchExecutionFailure(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchExecutionFailure", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractExecutionFailure, error) {
			...
			log.Infof("startupFn: Starting WatchExecutionFailure: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchExecutionFailure started: addr: %v", w.contractAddress.Hex())
			return sub, recvChan, nil
		}
		sub, recvChan, err := startupFn()
		if err != nil {
		...
		for {
			...
			// 30 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of ExecutionFailure")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting ExecutionFailure: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old ExecutionFailure")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart ExecutionFailure: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
...
func (w *MultiSigWalletContractWatchers) startWatchOwnerAddition(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchOwnerAddition", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractOwnerAddition, error) {
			...
			log.Infof("startupFn: Starting WatchOwnerAddition: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchOwnerAddition started: addr: %v", w.contractAddress.Hex())
			return sub, recvChan, nil
		}
		sub, recvChan, err := startupFn()
		if err != nil {
		...
		for {
			...
			// 30 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of OwnerAddition")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting OwnerAddition: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old OwnerAddition")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart OwnerAddition: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
...
func (w *MultiSigWalletContractWatchers) startWatchOwnerRemoval(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchOwnerRemoval", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractOwnerRemoval, error) {
			...
			log.Infof("startupFn: Starting WatchOwnerRemoval: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchOwnerRemoval started: addr: %v", w.contractAddress.Hex())
			return sub, recvChan, nil
		}
		sub, recvChan, err := startupFn()
		if err != nil {
		...
		for {
			...
			// 30 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of OwnerRemoval")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting OwnerRemoval: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old OwnerRemoval")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart OwnerRemoval: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
...
func (w *MultiSigWalletContractWatchers) startWatchRequirementChange(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchRequirementChange", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractRequirementChange, error) {
			...
			log.Infof("startupFn: Starting WatchRequirementChange: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchRequirementChange started: addr: %v", w.contractAddress.Hex())
			return sub, recvChan, nil
		}
		sub, recvChan, err := startupFn()
		if err != nil {
		...
		for {
			...
			// 30 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of RequirementChange")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting RequirementChange: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old RequirementChange")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart RequirementChange: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
...
func (w *MultiSigWalletContractWatchers) startWatchRevocation(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchRevocation", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractRevocation, error) {
			...
			log.Infof("startupFn: Starting WatchRevocation: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchRevocation started: addr: %v", w.contractAddress.Hex())
			return sub, recvChan, nil
		}
		sub, recvChan, err := startupFn()
		if err != nil {
		...
		for {
			...
			// 30 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of Revocation")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting Revocation: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old Revocation")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart Revocation: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
...
func (w *MultiSigWalletContractWatchers) startWatchSubmission(eventRecvChan chan *model.Event) (utils.WatcherSubscription, error) {
	...
	return utils.NewWatcherSubscription("WatchSubmission", func(quit <-chan struct{}) error {
		startupFn := func() (utils.WatcherSubscription, chan *contract.MultiSigWalletContractSubmission, error) {
			...
			log.Infof("startupFn: Starting WatchSubmission: addr: %v", w.contractAddress.Hex())
			...
			log.Infof("startupFn: WatchSubmission started: addr: %v", w.contractAddress.Hex())
			return sub, recvChan, nil
		}
		sub, recvChan, err := startupFn()
		if err != nil {
		...
		for {
			...
			// 30 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(preemptiveTimeoutSecs)):
				log.Infof("Premptive restart of Submission")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("Error starting Submission: addr: %v, %v", w.contractAddress.Hex(), err)
					w.errors <- err
					return err
				}
				log.Infof("Attempting to unsub old Submission")
				oldSub.Unsubscribe()
				log.Infof("Done preemptive restart Submission: addr: %v", w.contractAddress.Hex())
			case event := <-recvChan:
				if log.V(2) {
