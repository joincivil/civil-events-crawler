...
// File was generated at 2019-03-05 02:17:46.204308 +0000 UTC
package watcher
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CivilPLCRVotingContractPollCreated, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchPollCreated")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchPollCreated")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchPollCreated: %v", err)
					continue
				}
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CivilPLCRVotingContractTokensRescued, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchTokensRescued")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchTokensRescued")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchTokensRescued: %v", err)
					continue
				}
				return sub, recvChan, nil
			}
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CivilPLCRVotingContractVoteCommitted, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchVoteCommitted")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchVoteCommitted")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchVoteCommitted: %v", err)
					continue
				}
				return sub, recvChan, nil
			}
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CivilPLCRVotingContractVoteRevealed, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchVoteRevealed")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchVoteRevealed")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchVoteRevealed: %v", err)
					continue
				}
				return sub, recvChan, nil
			}
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CivilPLCRVotingContractVotingRightsGranted, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchVotingRightsGranted")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchVotingRightsGranted")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchVotingRightsGranted: %v", err)
					continue
				}
				return sub, recvChan, nil
			}
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		startupFn := func() (event.Subscription, chan *contract.CivilPLCRVotingContractVotingRightsWithdrawn, error) {
			...
			for {
				...
				log.Infof("WATCHER: startupFn: Starting WatchVotingRightsWithdrawn")
				...
				if err != nil {
					if sub != nil {
						log.Infof("WATCHER: startupFn: Unsubscribing WatchVotingRightsWithdrawn")
						sub.Unsubscribe()
					}
					if retry >= maxRetries {
						return nil, nil, err
					...
					retry++
					log.Warningf("WATCHER: startupFn: Retrying start WatchVotingRightsWithdrawn: %v", err)
					continue
				}
				return sub, recvChan, nil
			}
