...
// File was generated at 2019-03-03 23:56:11.773866 +0000 UTC
package watcher
	...
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of PollCreated")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting PollCreated: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart PollCreated: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
...
func (w *CivilPLCRVotingContractWatchers) startWatchTokensRescued(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of TokensRescued")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting TokensRescued: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart TokensRescued: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
...
func (w *CivilPLCRVotingContractWatchers) startWatchVoteCommitted(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of VoteCommitted")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting VoteCommitted: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart VoteCommitted: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
...
func (w *CivilPLCRVotingContractWatchers) startWatchVoteRevealed(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of VoteRevealed")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting VoteRevealed: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart VoteRevealed: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
...
func (w *CivilPLCRVotingContractWatchers) startWatchVotingRightsGranted(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of VotingRightsGranted")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting VotingRightsGranted: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart VotingRightsGranted: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
...
func (w *CivilPLCRVotingContractWatchers) startWatchVotingRightsWithdrawn(eventRecvChan chan *model.Event) (event.Subscription, error) {
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			// 15 min premptive resubscribe
			case <-time.After(time.Second * time.Duration(60*15)):
				// log.Infof("WATCHER: Premptive restart of VotingRightsWithdrawn")
				oldSub := sub
				sub, recvChan, err = startupFn()
				if err != nil {
					log.Errorf("WATCHER: Error starting VotingRightsWithdrawn: %v", err)
					return err
				}
				oldSub.Unsubscribe()
				log.Infof("WATCHER: Done preemptive restart VotingRightsWithdrawn: oldsub: %v, new sub: %v", oldSub, sub)
			case event := <-recvChan:
