...
// File was generated at 2019-12-19 21:31:48.911101 +0000 UTC
package filterer
...
func (f *MultiSigWalletContractFilterers) startFilterConfirmation(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract Confirmation for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract Confirmation for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	log.Infof("MultiSigWalletContract Confirmation added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterDeposit(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract Deposit for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract Deposit for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract Deposit added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterExecution(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract Execution for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract Execution for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract Execution added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterExecutionFailure(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract ExecutionFailure for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract ExecutionFailure for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract ExecutionFailure added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterOwnerAddition(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract OwnerAddition for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract OwnerAddition for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract OwnerAddition added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterOwnerRemoval(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract OwnerRemoval for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract OwnerRemoval for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract OwnerRemoval added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterRequirementChange(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract RequirementChange for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract RequirementChange for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract RequirementChange added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterRevocation(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract Revocation for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract Revocation for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract Revocation added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *MultiSigWalletContractFilterers) startFilterSubmission(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering MultiSigWalletContract Submission for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: MultiSigWalletContract Submission for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("MultiSigWalletContract Submission added: %v", numEventsAdded)
	return pastEvents, nil
}
