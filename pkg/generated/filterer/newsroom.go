...
// File was generated at 2018-10-30 22:34:29.254330066 +0000 UTC
package filterer
...
func (f *NewsroomContractFilterers) startFilterOwnershipRenounced(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("OwnershipRenounced", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt64(), model.Filterer)
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
