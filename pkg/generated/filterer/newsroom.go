...
// File was generated at 2018-06-25 17:11:08.580641778 +0000 UTC
package filterer
...
func (f *NewsroomContractFilterers) startFilterContentPublished(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("ContentPublished", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
...
func (f *NewsroomContractFilterers) startFilterNameChanged(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("NameChanged", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterOwnershipTransferred(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("OwnershipTransferred", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRevisionSigned(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("RevisionSigned", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRevisionUpdated(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("RevisionUpdated", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRoleAdded(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("RoleAdded", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRoleRemoved(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("RoleRemoved", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
