...
// File was generated at 2018-07-12 18:54:11.701545215 +0000 UTC
package filterer
...
func (f *NewsroomContractFilterers) startFilterContentPublished(startBlock uint64, pastEvents []*model.CivilEvent) (error, []*model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEventFromContractEvent("ContentPublished", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
...
func (f *NewsroomContractFilterers) startFilterNameChanged(startBlock uint64, pastEvents []*model.CivilEvent) (error, []*model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEventFromContractEvent("NameChanged", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterOwnershipTransferred(startBlock uint64, pastEvents []*model.CivilEvent) (error, []*model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEventFromContractEvent("OwnershipTransferred", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRevisionSigned(startBlock uint64, pastEvents []*model.CivilEvent) (error, []*model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEventFromContractEvent("RevisionSigned", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRevisionUpdated(startBlock uint64, pastEvents []*model.CivilEvent) (error, []*model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEventFromContractEvent("RevisionUpdated", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRoleAdded(startBlock uint64, pastEvents []*model.CivilEvent) (error, []*model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEventFromContractEvent("RoleAdded", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRoleRemoved(startBlock uint64, pastEvents []*model.CivilEvent) (error, []*model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEventFromContractEvent("RoleRemoved", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
