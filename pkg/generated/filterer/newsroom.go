...
// File was generated at 2018-07-20 19:06:09.075407068 +0000 UTC
package filterer
...
func (f *NewsroomContractFilterers) startFilterContentPublished(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("ContentPublished", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
...
func (f *NewsroomContractFilterers) startFilterNameChanged(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("NameChanged", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterOwnershipTransferred(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("OwnershipTransferred", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRevisionSigned(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("RevisionSigned", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRevisionUpdated(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("RevisionUpdated", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRoleAdded(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("RoleAdded", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *NewsroomContractFilterers) startFilterRoleRemoved(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("RoleRemoved", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
