...
// File was generated at 2019-12-19 21:31:46.854756 +0000 UTC
package filterer
...
func (f *GovernmentContractFilterers) startFilterAppellateSet(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering GovernmentContract AppellateSet for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: GovernmentContract AppellateSet for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	log.Infof("GovernmentContract AppellateSet added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *GovernmentContractFilterers) startFilterGovtReparameterizationProposal(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering GovernmentContract GovtReparameterizationProposal for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: GovernmentContract GovtReparameterizationProposal for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("GovernmentContract GovtReparameterizationProposal added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *GovernmentContractFilterers) startFilterNewConstSet(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering GovernmentContract NewConstSet for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: GovernmentContract NewConstSet for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("GovernmentContract NewConstSet added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *GovernmentContractFilterers) startFilterParameterSet(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering GovernmentContract ParameterSet for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: GovernmentContract ParameterSet for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("GovernmentContract ParameterSet added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *GovernmentContractFilterers) startFilterProposalExpired(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering GovernmentContract ProposalExpired for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: GovernmentContract ProposalExpired for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("GovernmentContract ProposalExpired added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *GovernmentContractFilterers) startFilterProposalFailed(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering GovernmentContract ProposalFailed for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: GovernmentContract ProposalFailed for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("GovernmentContract ProposalFailed added: %v", numEventsAdded)
	return pastEvents, nil
}
...
func (f *GovernmentContractFilterers) startFilterProposalPassed(startBlock uint64, pastEvents []*model.Event) ([]*model.Event, error) {
	...
	log.Infof("Filtering GovernmentContract ProposalPassed for %v at block %v", f.contractAddress.Hex(), startBlock)
	...
	for {
		...
		if err == nil {
			log.Infof("Done filter: GovernmentContract ProposalPassed for %v", f.contractAddress.Hex())
			break
		}
		if retry >= maxRetries {
	...
	numEventsAdded := len(pastEvents) - beforeCount
	log.Infof("GovernmentContract ProposalPassed added: %v", numEventsAdded)
	return pastEvents, nil
}
