...
// File was generated at 2018-06-25 04:34:52.936861458 +0000 UTC
package filterer
...
func (f *CivilTCRContractFilterers) startFilterAppealGranted(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("AppealGranted", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
...
func (f *CivilTCRContractFilterers) startFilterAppealRequested(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("AppealRequested", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterApplication(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("Application", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterApplicationRemoved(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("ApplicationRemoved", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterApplicationWhitelisted(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("ApplicationWhitelisted", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterChallenge(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("Challenge", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterChallengeFailed(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("ChallengeFailed", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterChallengeSucceeded(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("ChallengeSucceeded", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterDeposit(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("Deposit", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterFailedChallengeOverturned(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("FailedChallengeOverturned", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGovernmentTransfered(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("GovernmentTransfered", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealChallenged(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("GrantedAppealChallenged", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealConfirmed(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("GrantedAppealConfirmed", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealOverturned(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("GrantedAppealOverturned", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterListingRemoved(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("ListingRemoved", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterListingWithdrawn(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("ListingWithdrawn", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterRewardClaimed(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("RewardClaimed", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterSuccessfulChallengeOverturned(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("SuccessfulChallengeOverturned", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterTouchAndRemoved(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("TouchAndRemoved", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterWithdrawal(startBlock uint64, pastEvents []model.CivilEvent) (error, []model.CivilEvent) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent, err := model.NewCivilEvent("Withdrawal", f.ContractName(), f.contractAddress, itr.Event)
		if err != nil {
			log.Errorf("Error creating new civil event: event: %v, err: %v", itr.Event, err)
			continue
		}
