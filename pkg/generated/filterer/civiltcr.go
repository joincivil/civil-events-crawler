...
// File was generated at 2018-07-20 19:06:07.941424408 +0000 UTC
package filterer
...
func (f *CivilTCRContractFilterers) startFilterAppealGranted(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("AppealGranted", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
...
func (f *CivilTCRContractFilterers) startFilterAppealRequested(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("AppealRequested", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterApplication(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("Application", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterApplicationRemoved(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("ApplicationRemoved", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterApplicationWhitelisted(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("ApplicationWhitelisted", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterChallenge(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("Challenge", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterChallengeFailed(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("ChallengeFailed", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterChallengeSucceeded(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("ChallengeSucceeded", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterDeposit(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("Deposit", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterFailedChallengeOverturned(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("FailedChallengeOverturned", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGovernmentTransfered(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("GovernmentTransfered", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealChallenged(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("GrantedAppealChallenged", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealConfirmed(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("GrantedAppealConfirmed", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealOverturned(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("GrantedAppealOverturned", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterListingRemoved(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("ListingRemoved", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterListingWithdrawn(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("ListingWithdrawn", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterRewardClaimed(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("RewardClaimed", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterSuccessfulChallengeOverturned(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("SuccessfulChallengeOverturned", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterTouchAndRemoved(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("TouchAndRemoved", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
...
func (f *CivilTCRContractFilterers) startFilterWithdrawal(startBlock uint64, pastEvents []*model.Event) (error, []*model.Event) {
	...
	nextEvent := itr.Next()
	for nextEvent {
		modelEvent, err := model.NewEventFromContractEvent("Withdrawal", f.ContractName(), f.contractAddress, itr.Event, utils.CurrentEpochSecsInInt())
		if err != nil {
			log.Errorf("Error creating new event: event: %v, err: %v", itr.Event, err)
			continue
		}
