...
// File was generated at 2018-06-14 20:47:36.830030282 +0000 UTC
package filterer
...
func (f *CivilTCRContractFilterers) startFilterAppealGranted(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("AppealGranted", f.contractAddress, itr.Event)
...
func (f *CivilTCRContractFilterers) startFilterAppealRequested(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("AppealRequested", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterApplication(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("Application", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterApplicationRemoved(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("ApplicationRemoved", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterApplicationWhitelisted(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("ApplicationWhitelisted", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterChallenge(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("Challenge", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterChallengeFailed(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("ChallengeFailed", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterChallengeSucceeded(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("ChallengeSucceeded", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterDeposit(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("Deposit", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterFailedChallengeOverturned(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("FailedChallengeOverturned", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterGovernmentTransfered(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("GovernmentTransfered", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealChallenged(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("GrantedAppealChallenged", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealConfirmed(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("GrantedAppealConfirmed", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterGrantedAppealOverturned(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("GrantedAppealOverturned", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterListingRemoved(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("ListingRemoved", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterListingWithdrawn(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("ListingWithdrawn", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterRewardClaimed(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("RewardClaimed", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterSuccessfulChallengeOverturned(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("SuccessfulChallengeOverturned", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterTouchAndRemoved(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("TouchAndRemoved", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
...
func (f *CivilTCRContractFilterers) startFilterWithdrawal(startBlock uint64, pastEvents *[]model.CivilEvent) error {
	...
	nextEvent := itr.Next()
	for nextEvent {
		civilEvent := model.NewCivilEvent("Withdrawal", f.contractAddress, itr.Event)
		*pastEvents = append(*pastEvents, *civilEvent)
		nextEvent = itr.Next()
	}
	return nil
